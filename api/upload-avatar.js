...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get userId from form (so we know which avatar to overwrite)
    const userId = req.body.userId || req.query.userId;
    if (!userId) return res.status(400).send("Missing userId");

    const publicId = `avatar_${userId}`;

    // Upload to Cloudinary, overwrite the previous avatar
    const result = await cloudinary.v2.uploader.upload(file.path, {
      folder: "avatars",
      public_id: publicId,
      overwrite: true, // replaces the old one
      invalidate: true // clears cached versions
    });
