...
const supabase = createClient(
  ...
  "sb_secret_HRrrL83uTSSDXlbksd-7Vw_yzpPS0Y9" // Supabase Service Role Key
);
...
export default async function startRecording(req, res) {
  try {
    ...
    if (!profile || !profile.videosdk_room_id || !profile.videosdk_host_token) {
      return res.status(400).json({ success: false, error: "User has no active stream info" });
    }
    ...
    console.log("🎬 VideoSDK raw response:", text);

    let data;
    try {
      data = JSON.parse(text);
    } catch {
      return res.status(500).json({ success: false, error: "Invalid JSON from VideoSDK", raw: text });
    }

    if (!response.ok || !data?.data?.id) {
      return res.status(500).json({ success: false, error: "Start recording failed", details: data });
    }

    // ✅ Success
    const recordingId = data.data.id;
    return res.status(200).json({ success: true, recordingId, message: "Recording started successfully" });
