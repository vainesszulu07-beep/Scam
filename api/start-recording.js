...
const supabase = createClient(
  ...
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVpbWVjdWlpeHdnbXBmcGVkeHByIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MTA0MjQ3NCwiZXhwIjoyMDg2NjE4NDc0fQ.735Jlm22UZ3JICXCxsP5CyVIN-Bsn4j0XqFTU61bILg" // Server-side key
);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!roomId || !hostToken) {
      logs.push("❌ Profile missing roomId or hostToken. Recording cannot start.");
      return res.status(500).json({ success: false, logs });
    }

    logs.push(`📡 Using roomId: ${roomId}`);

    // 3️⃣ Start VideoSDK recording
    logs.push("🎬 Calling VideoSDK start recording...");
    const options = {
      method: "POST",
      headers: {
