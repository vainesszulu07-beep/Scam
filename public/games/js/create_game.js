...
async function boot(session, deps) {
  ...
  try {
    ...
    if (activePlayingGame) {
      ...
      if (activeGameType) {
        await deps.openGameTypeSheet(activeGameType, activePlayingGame);
    ...
    if (state.myGame && state.myGame.status !== "idle") {
      ...
      if (error) throw error;
      if (resetGame) state.myGame = resetGame;
    }
